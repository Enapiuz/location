...
/**
 * The IpGeoBase driver.
 *
 * Class IpGeoBaseRu
 */
class IpGeoBaseRu implements DriverInterface
{
    /**
    ...
    public function __construct(LocationInstance $instance)
    {
        ...
        $this->url = $this->config->get('location'.$this->instance->getConfigSeparator().'drivers.IpGeoBaseRu.url');
    }
    ...
    public function get($ip)
    {
        ...
        try {
            $contents = file_get_contents($this->url.$ip);

            $parser = xml_parser_create();
            $parsed = xml_parse($parser, $contents);
